...
public struct AuthBrandHeader: View {
    ...
    let subtitle: String
    var compact: Bool
    /// Host-supplied brand mark. When `nil`, a neutral SF Symbol is shown so
    /// FeatureAuth never depends on any specific app's artwork.
    let logo: Image?

    public init(title: String = "Script Builder",
                subtitle: String,
                compact: Bool = false,
                logo: Image? = nil) {
        self.title = title
    ...
    /// Resolves the injected logo, or falls back to a neutral placeholder.
    private var logoImage: Image {
        logo ?? Image(systemName: "person.crop.circle.badge.checkmark")
    }
